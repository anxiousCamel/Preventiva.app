// js/config.js
export const lojas = [
    { value: 'rebouças',       label: 'Rebouças' },
    { value: 'ivo',            label: 'Ivo' },
    { value: 'bom_retiro',     label: 'Bom Retiro' },
    { value: 'hortolandia',    label: 'Hortolândia' },
    { value: 'matao',          label: 'Matão' },
    { value: 'maria_antonia',  label: 'Maria Antônia' },
    { value: 'monte_mor',      label: 'Monte Mor' },
    { value: 'indaiatuba',     label: 'Indaiatuba' },
    { value: 'mogi_mirim',     label: 'Mogi Mirim' },
    { value: 'sousas',         label: 'Sousas' },
    { value: 'taquaral',       label: 'Taquaral' },
    { value: 'mogi_guacu',     label: 'Mogi Guaçu' },
    { value: 'bordon',         label: 'Bordon' },
  ];
  
  export const setores = [
    { value: 'Açougue',         label: 'Açougue' },
    { value: 'Padaria',         label: 'Padaria' },
    { value: 'Frios',           label: 'Frios e Laticínios' },
    { value: 'Hortifruti',      label: 'Hortifruti' },
    { value: 'Rotisseria',      label: 'Rotisseria' },
    { value: 'Peixaria',        label: 'Peixaria' },
    { value: 'Checkout',        label: 'Frente de loja' },
  ];
  
  export const tipos = [
    { value: 'balanca_setor',    label: 'Balança setor' },
    { value: 'balanca_checkout', label: 'Balança checkout' },
    { value: 'impressora',       label: 'Impressora' },
    { value: 'teclado',          label: 'Teclado' },
    { value: 'gabinete',         label: 'Gabinete' },
    { value: 'gaveta',           label: 'Gaveta' },
    { value: 'scanner',          label: 'Scanner' },
  ];

  export const checklists = {
  'balanca_setor': [
    { idSuffix: "limparCabecote", labelText: "Limpar cabeçote" },
    { idSuffix: "testeDeImpressao", labelText: "Teste de impressão" },
    { idSuffix: "peso", labelText: "Peso da balança" },
    { idSuffix: "teclado", labelText: "Teclado" },
    { idSuffix: "display", labelText: "Display" },
    { idSuffix: "nivelarBalanca", labelText: "Nivelar balança" },
    { idSuffix: "cabos", labelText: "Cabos" },
    { idSuffix: "retirarAdesivos", labelText: "Retirar adesivo e outros" },
    { idSuffix: "tampas", labelText: "Tampas" },
    { idSuffix: "limpezaBalanca", labelText: "Limpeza da balança" }
  ],
  'balanca_checkout': [
    // você pode reutilizar o mesmo da setor se quiser
    { idSuffix: "peso", labelText: "Peso da balança" },
    { idSuffix: "display", labelText: "Display" },
    { idSuffix: "nivelarBalanca", labelText: "Nivelar balança" },
    { idSuffix: "cabos", labelText: "Cabos" },
    { idSuffix: "tampas", labelText: "Tampas" },
    { idSuffix: "limpezaBalanca", labelText: "Limpeza da balança" }
  ],
  'impressora': [
    { idSuffix: "limparCabecote", labelText: "Limpeza cabeça de impressão" },
    { idSuffix: "testeDeImpressaoCheque", labelText: "Teste Fita de impressão cheque" },
    { idSuffix: "testeDeImpressaoCupom", labelText: "Teste impressão cupom" },
    { idSuffix: "ProteçãoLacre", labelText: "Proteção lacre com fita" },
    { idSuffix: "Higienizacao", labelText: "Higienização" }
  ]
  // adicione outros tipos conforme necessário
};

  
  